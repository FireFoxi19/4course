...
def ElGamal_signcheck(m: bytearray, s: list):
    ...
    # числовое представление хэш функции для проверки подлинности
    h_b = ''.join(str(pow_module(g, int(i, 16), p)) for i in h)
    print(f'ElGamal md5 числовое представление хэша: {h_b}\n')
    res = ''.join(str(pow_module(y, r, p) * pow_module(r, i, p) % p) for i in s)
    print(f'ElGamal проверка подлиности: {res}\n')

    print("Результат проверки подлинности: " + ((Fore.CYAN + 'same') if res == h_b else (Fore.RED + 'fake')) + Fore.RESET+'\n')
...
def RSA_sign(m) -> list:
    ...
    h = hashlib.md5(m).hexdigest()
    print('Sign:\n')
    print(f'RSA md5 Хэш: {h}\n')
    # числовое представление хэш функции для проверки подлинности
    h_b = ''.join(str(int(i, 16)) for i in h)
    print(f'RSA md5 числовое представление хэша: {h_b}\n')
    # наша подпись
...
def RSA_signcheck(m: bytearray, s: list):
    ...
    print(f'RSA проверка подлиности: {e}\n')

    print("Результат проверки подлинности: " + ((Fore.CYAN + 'same') if e == h_b else (Fore.RED + 'fake')) + Fore.RESET+'\n')
...
def GOST_sign(m: bytearray) -> bool:
    ...
    with open(r'C:\ZI\signs\GOST_sign.txt', 'w') as f:
        f.write(str(s))
...
def GOST_signcheck(m: bytearray, s: list):
    ...
    print("Результат проверки подлинности: " + ((Fore.CYAN + 'same') if v == r else (Fore.RED + 'fake')) + Fore.RESET+'\n')


if __name__ == '__main__':
    try:
        shutil.rmtree('C:\ZI\signs')
    except OSError:
        pass
    os.mkdir('C:\ZI\signs')
    filename = 'C:\ZI\input'
    filename_fake = "C:\ZI\input1"
    ext = 'txt'
    ...
    with open(r'C:\ZI\signs\rsa_sign.txt', 'w') as f:
        f.write(str(s))
    RSA_signcheck(m, s)
    print()
    s = ElGamal_sign(m)
    with open(r'C:\ZI\signs\elgamal_sign.txt', 'w') as f:
        f.write(str(s))
