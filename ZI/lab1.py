...
def giant_baby_step(a, p, y):
    ...
    #print("Шаг младенца ", baby)
    giant = [pow_module(a, (m * i), p) for i in range(1, k + 1)]
    #print("Шаг великана ", giant)
...
if __name__ == '__main__':
    ...
    # print(gcd(28,19))
    #print("Обобщенный евклид",gcd_modified(generate_prime(1, 7), generate_prime(1, 7)))
    #print("Метод Евклида ", gcd(2, 3))
    #print("Быстрое возведение в степень ", pow_module(random.randint(1, 10**8), random.randint(1, 10**8), random.randint(1, 10**4)))
   # print("Генерация простого числа ", generate_prime(1, 7))
    diffie_hellman_algorithm()
    a = random.randint(1, 10**8)  
    x = random.randint(1, 10**8)  
    print("x = ", x)
    p = random.randint(1, 10**4)  
    result = pow_module(a, x, p)  

    print("Быстрое возведение в степень ", result)

    x = giant_baby_step(a, p, result)
    # if x is not None:
    #     #print("Найденное значение x:", x)
    #     #print("Нахождение второго у: ", pow_module(a,x,p))
    # else:
    #     #print("Решение не найдено.")

    #print("Шаг младенца шаг великана ", giant_baby_step(88, 107, 47))