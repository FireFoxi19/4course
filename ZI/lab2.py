...
def ElGamal_encode(m) -> list:
    ...
    print("g = ", g)
    x = generate_prime(0, p - 1) # закрытый
    print("x = ", x)
    y = pow_module(g, x, p) # открытый
    print("y = ", y)

    k = generate_prime(0, p - 2) # секретный
    print("k = ", k)
    a = pow_module(g, k, p) # открытый
    print("a = ", a)
    # для теста
    # y = 21
    # x = 13
